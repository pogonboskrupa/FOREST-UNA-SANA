...
param(
    [ValidateSet("codex-forest", "apk-build-v1")]
    [string]$Branch  = "codex-forest",
...
Write-Host "`n=== UNA SANA FOREST APK Builder ===" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0 -or $CurrentBranch -ne $Branch) {
    throw "Build se pokreće samo sa $Branch. Skripta ne mijenja grane."
}
...
if ($ApkPath) {
    ...
    $DestName = "UNA-SANA-FOREST-v$GradleVer-$BuildType-$(Get-Date -Format 'yyyyMMdd-HHmm').apk"
